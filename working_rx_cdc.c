...
char        konvertuotas_paketas[10];   // buferis skirtas saugoti gauta paketa.
...
// Write received data to port 1 on CRC match
    ...
    {
      // patikrinam ar visi paketo elementai gauti. Jei taip - spausdinam juos.
      if(packet_buf[0] >= 9)
      {
        sprintf(konvertuotas_paketas,"%s", packet_buf);

        size_t size = sprintf(m_tx_buffer,konvertuotas_paketas, frame_counter);
        ret = app_usbd_cdc_acm_write(&m_app_cdc_acm, m_tx_buffer, size);
      }
    }
  ...
  //  dbg_handler();
 nrf_delay_ms(300); // Jokiu budu nevirsyt 500ms, nes kyla problemu su hex irasymu ir ismeta corrupted device.
  }// WHILE END
    }// MAIN END
